...
type PromiseAjax = Promise<
  AxiosResponse<{ page: number; results: PropsMovie[]; total_pages: number }>
>;
type TaxGetAllShowing = (pg?: number) => PromiseAjax;
const axGetAllShowing: TaxGetAllShowing = (pg = 1) => {
  return axios({
    url: `https://api.themoviedb.org/3/movie/now_playing?api_key=${api_key}&language=en-US&page=${pg}`,
    method: 'GET'
  });
};
